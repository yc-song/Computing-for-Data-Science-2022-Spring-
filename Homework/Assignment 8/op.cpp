...
Tensor* Softmax::backward(Tensor* out){
  ...
  for(int n=0; n<N; ++n){
    ...
    grad_val[0]=p0*(1-p0)*(out->getGrad(n,0))-p0*p1*(out->getGrad(n,1))-p0*p2*(out->getGrad(n,2));
    grad_val[1]=p1*(1-p1)*(out->getGrad(n,1))-p0*p1*(out->getGrad(n,0))-p1*p2*(out->getGrad(n,2));
    grad_val[2]=p2*(1-p2)*(out->getGrad(n,2))-p0*p2*(out->getGrad(n,0))-p1*p2*(out->getGrad(n,1));
...
Tensor* Linear::backward(Tensor* out){	
  ...
  for(int n=0; n<N; ++n){
    for(int i=0; i<ic_; ++i){
      ...
      for(int o=0; o<oc_; ++o){
        // tmp+=weight_->getData(i,o)*out->getGrad(i,o);
        tmp+=weight_->getData(i,o)*out->getGrad(n,o);
  ...
  for(int i=0; i<ic_; ++i){
    for(int o=0; o<oc_; ++o){
      ...
      for(int n=0; n<N; ++n){
        //tmp_grad+=out_->getData(i,o,n)*out->getGrad(n,i);
        // tmp_grad+=in_->getData(n,i)*out->getGrad(i,o);
        tmp_grad+=in_->getData(n,i)*out->getGrad(n,o);
...
void Loss::backward(Tensor *out){
  double dloss = 1.0/(double)(n_);
  double grad;
  double grad1;
  for (int i =0;i<n_;++i){
    if(ctx_[i*6]==1){
    grad=-dloss/ctx_[i*6+3];
    out->setGrad(i,0,grad);
    out->setGrad(i,1,0);
    out->setGrad(i,2,0);
    }
    else if(ctx_[i*6+1]==1){
    grad=-dloss/ctx_[i*6+4];
    out->setGrad(i,1,grad);
    out->setGrad(i,0,0);
    out->setGrad(i,2,0);
    }
    else if(ctx_[i*6+2]==1){
    grad=-dloss/ctx_[i*6+5];
    out->setGrad(i,2,grad);
    out->setGrad(i,0,0);
    out->setGrad(i,1,0);
    }
    //     if(ctx_[i*6]==1){
    // grad=-dloss/ctx_[i*6+3];
    // out->setGrad(i,0,grad);
    // out->setGrad(i,1,-dloss/(1-ctx_[i*6+4]));
    // out->setGrad(i,2,-dloss/(1-ctx_[i*6+5]));
    // }
    // else if(ctx_[i*6+1]==1){
    // grad=-dloss/ctx_[i*6+4];
    // out->setGrad(i,1,grad);
    // out->setGrad(i,0,-dloss/(1-ctx_[i*6+3]));
    // out->setGrad(i,2,-dloss/(1-ctx_[i*6+5]));
    // }
    // else if(ctx_[i*6+2]==1){
    // grad=-dloss/ctx_[i*6+5];
